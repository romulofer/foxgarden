...
fn draft_path(project_root: &Path, file_path: &Path) -> Option<PathBuf> {
    ...
    name.push(".draft");
    Some(project_root.join(".foxgarden").join("drafts").join(relative.parent()?).join(name))
}
...
fn collect(dir: &Path, drafts_root: &Path, project_root: &Path, found: &mut Vec<Draft>) {
    ...
    for entry in entries.flatten() {
        ...
        let Some(file_name) = relative.file_name().and_then(|n| n.to_str()).and_then(|n| n.strip_suffix(".draft"))
        else {
            continue;
        };
...
mod tests {
    ...
    fn a_draft_matching_disk_is_not_offered_and_is_cleaned_up() {
        ...
        assert!(pending_drafts(&root).is_empty(), "nothing was lost, so there is nothing to restore");
        assert!(pending_drafts(&root).is_empty());
        assert!(!root.join(".foxgarden/drafts/Main.java.draft").exists(), "and the stale draft is gone");
    }
