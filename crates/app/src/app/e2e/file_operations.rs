...
fn renaming_a_file_from_the_tree_renames_it_on_disk_and_repoints_its_tab() {
    ...
    assert!(app.shows("☕ Renamed.java"), "and so must the tree row");
}
...
fn deleting_a_file_asks_first_then_removes_it_and_closes_its_tab() {
    ...
    assert!(app.shows("🔷 Other.kt"), "its siblings stay put");
}
...
fn ctrl_clicking_two_rows_deletes_both_in_one_confirmation() {
    ...
    app.click_modifiers("☕ A.java", egui::Modifiers::COMMAND);
    app.click_modifiers("☕ B.java", egui::Modifiers::COMMAND);
