...
pub fn coverage_covered(dark_mode: bool) -> Color32 {
    if dark_mode {
        DARK_COVERAGE_COVERED
    } else {
        LIGHT_COVERAGE_COVERED
    }
}

pub fn coverage_missed(dark_mode: bool) -> Color32 {
    if dark_mode {
        DARK_COVERAGE_MISSED
    } else {
        LIGHT_COVERAGE_MISSED
    }
}
...
pub fn run_marker_hovered(dark_mode: bool) -> Color32 {
    if dark_mode {
        DARK_RUN_MARKER_HOVERED
    } else {
        LIGHT_RUN_MARKER_HOVERED
    }
}

pub fn debug_current_line(dark_mode: bool) -> Color32 {
    if dark_mode {
        DARK_DEBUG_CURRENT_LINE
    } else {
        LIGHT_DEBUG_CURRENT_LINE
    }
}

pub fn coverage_partial(dark_mode: bool) -> Color32 {
    if dark_mode {
        DARK_COVERAGE_PARTIAL
    } else {
        LIGHT_COVERAGE_PARTIAL
    }
}
