...
use crate::widgets::editor::{
    self, AccessorKind, CompletionState, GenerateAccessorsDialog, GenerateMethodDialog, GenerateMethodKind, HoverState,
    OverrideMethodDialog, PeekState, UserTemplates,
};
...
pub fn show(
                                ...
                                label_response.context_menu(|ui| {
                                    ...
                                    if ui
                                        .add_enabled(
                                            ...
                                            egui::Button::new(t().tabs.close_others),
                                        )
                                        .clicked()
                                    {
    ...
    if is_split && let Some(pos) = ui.input(|i| i.pointer.any_pressed().then(|| i.pointer.press_origin()).flatten()) {
