...
impl JdkRegistryState {
    ...
    pub fn poll_auto_detect(&mut self) -> Vec<JavaRuntime> {
        let Some(rx) = self.auto_detect_rx.as_ref() else {
            return Vec::new();
        };
        match rx.try_recv() {
            Ok(runtimes) => {
...
pub fn show_settings(ui: &egui::Ui, state: &mut JdkRegistryState, registry: &mut JdkRegistry) {
    ...
    let outcome = show_modal(
        ...
        |ui, ()| {
            ...
            ui.horizontal(|ui| {
                if ui
                    .add_enabled(!state.picker_running(), egui::Button::new(t().jdk.add))
                    .clicked()
                {
                    state.picker.open(None);
                }
                if ui
                    .add_enabled(!state.auto_detect_running(), egui::Button::new(t().jdk.auto_detect))
                    .clicked()
                {
