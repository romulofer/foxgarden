...
fn extract_zip_and_locate_launcher(tool: Tool, dir: &Path, downloaded: &[u8]) -> Result<PathBuf, String> {
    ...
    let top_level = std::fs::read_dir(dir)
        ...
        .find(|entry| entry.file_type().is_ok_and(|t| t.is_dir()) && entry.file_name().to_string_lossy().contains(hint))
        .ok_or_else(|| {
            format!(
                "couldn't find {}'s extracted directory under {}",
                tool.display_name(),
                dir.display()
            )
        })?
    ...
    if !script.exists() {
        return Err(format!(
            "expected launcher at {} but it doesn't exist",
            script.display()
        ));
    }
    Ok(script)
...
#[cfg(test)]
#[path = "tool_manager_test.rs"]
mod tool_manager_test;
