...
//! on the code under test. Behavior-identical to the inline module it replaced.
    ...
    fn sticky_headers_caps_at_max_depth_keeping_the_outermost() {
        ...
        assert_eq!(sticky_headers_to_pin(&[10, 20, 30, 40, 50, 60], 100, 5), vec![10, 20, 30, 40, 50]);
    }
    ...
    fn sticky_scroll_enabled_renders_without_panicking() {
        ...
        let view = ViewSettings { show_sticky_scroll: true, ..ViewSettings::default() };

        egui::__run_test_ui(|ui| {
            // EditorFont::Default, not JetBrainsMono: see the first test in
            // this file for why.
            show(ui, &mut doc, &mut parser, EditorFont::Default, 14.0, IndentSettings::default(), view, None, &mut None, None, &mut None, None, false, &mut None, None, false, false, &mut None, &mut Vec::new(), &mut None);
        });
    }
    ...
    fn whitespace_and_indent_guides_render_without_panicking() {
        ...
        let mut parser = parsed(Language::Java, &doc.buffer.to_string());
        let view_settings =
            ViewSettings { word_wrap: true, show_whitespace: true, show_indent_guides: true, show_sticky_scroll: false };
