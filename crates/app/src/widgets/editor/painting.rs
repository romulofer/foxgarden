...
pub(super) fn paint_paused_line_highlight(ui: &egui::Ui, out: &TextAreaOutput, line: usize, dark_mode: bool) {
    ...
    ui.painter()
        .rect_filled(rect, 0.0, theme::debug_current_line(dark_mode));
}
...
pub(super) fn blame_annotation_text(line: &BlameLine, now_unix: i64) -> String {
    ...
    format!(
        "{} • {} • {}",
        line.author,
        relative_time(now_unix, line.author_time),
        line.summary
    )
}
...
pub(super) fn paint_blame_annotation(
    ...
    let Some(i) = out.row_galleys.iter().position(|(logical, _)| *logical == cursor_line) else {
        return;
    };
