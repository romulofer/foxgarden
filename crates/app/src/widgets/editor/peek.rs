...
fn visible_window(panel: &PanelContent, expanded: bool) -> (usize, &[String]) {
    let radius = if expanded { EXPANDED_CONTEXT_LINES } else { COLLAPSED_CONTEXT_LINES };
...
fn resolve_panel(target: Target, current_doc: &Document) -> Option<PanelContent> {
    match target {
        Target::File { path, range } => {
            let text =
                if path == current_doc.path { current_doc.buffer.to_string() } else { std::fs::read_to_string(&path).ok()? };
...
fn panel_from_text(path: PathBuf, text: &str, byte_offset: usize) -> PanelContent {
    ...
    PanelContent { path, first_line: start + 1, target_line: target_index + 1, lines }
}
