...
fn incremental_reparse_matches_full_reparse() {
    ...
    let mut incremental_parser = IncrementalParser::new(Language::Java).expect("a bundled grammar must load");
...
fn valid_properties_has_no_syntax_errors() {
    let mut parser = IncrementalParser::new(Language::Properties).expect("a bundled grammar must load");
...
fn valid_yaml_has_no_syntax_errors() {
    let mut parser = IncrementalParser::new(Language::Yaml).expect("a bundled grammar must load");
...
fn valid_xml_has_no_syntax_errors() {
    let mut parser = IncrementalParser::new(Language::Xml).expect("a bundled grammar must load");
...
fn valid_dockerfile_has_no_syntax_errors() {
    let mut parser = IncrementalParser::new(Language::Dockerfile).expect("a bundled grammar must load");
