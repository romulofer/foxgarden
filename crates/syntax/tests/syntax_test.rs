...
/// A parser for `language`, with this build's shipped grammars installed
/// first.
///
/// Since `PLAN.md` Track 24 Phase 3 a grammar reaches the editor through
/// the extension registry rather than being linked into `syntax`, so a test
/// binary that never installs one parses nothing. The library's own unit
/// tests get this for free (`grammars::store` installs for them); an
/// integration test lives in its own crate and has to ask, which is what
/// this helper is. Installing is idempotent, so every test may call it.
fn parser(language: Language) -> IncrementalParser {
    static INSTALLED: std::sync::Once = std::sync::Once::new();
    INSTALLED.call_once(|| {
        let errors = syntax::install_grammars(&fg_languages::builtin_registry());
        assert!(errors.is_empty(), "shipped grammars must install: {errors:?}");
    });
    IncrementalParser::new(language).expect("an installed grammar must load")
}
...
fn valid_java_has_no_syntax_errors() {
    let mut parser = parser(Language::Java);
...
fn valid_kotlin_has_no_syntax_errors() {
    let mut parser = parser(Language::Kotlin);
...
fn incremental_reparse_matches_full_reparse() {
    ...
    let mut full_parser = parser(Language::Java);
    ...
    let mut incremental_parser = parser(Language::Java);
...
fn valid_properties_has_no_syntax_errors() {
    let mut parser = parser(Language::Properties);
...
fn valid_yaml_has_no_syntax_errors() {
    let mut parser = parser(Language::Yaml);
...
fn valid_xml_has_no_syntax_errors() {
    let mut parser = parser(Language::Xml);
...
fn valid_dockerfile_has_no_syntax_errors() {
    let mut parser = parser(Language::Dockerfile);
