...
- **Don't commit or push on your own initiative — only when the user
  explicitly asks for it in that moment.** This is per-request, not
  standing authorization for the rest of the session: a commit/push
  earlier in a conversation doesn't imply the next one is also wanted
  without being asked again.
- **Whenever a commit does happen, the user is the sole author — no
  `Co-Authored-By` trailer, no mention of the agent in the message.** This
  repo's commit history is attributed to the human, full stop, even when
  an agent typed the diff.
